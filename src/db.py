...
def add_progress(goal_id, actual_amount):
    query = "INSERT INTO progress_log (goal_id, actual_amount, date) VALUES (%s, %s, %s)"
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(query, (goal_id, actual_amount, date.today()))
            conn.commit()
            return True
    return False
...
def add_goal(user_id, goal_name, start_point, end_point, start_date, end_date):
    query = "INSERT INTO goals (user_id, goal_name, start_point, end_point, start_date, end_date) VALUES (%s, %s, %s, %s, %s, %s)"
    with get_connection() as conn:
        with conn.cursor() as cur:
