...
async def fetch_active_userlist(within_date):
    query = """
        SELECT DISTINCT u.id FROM users u
        JOIN goals g ON u.id = g.user_id
        LEFT JOIN progress_log p ON g.id = p.goal_id
        WHERE p.date >= CURRENT_DATE - (make_interval(days => $1))
           OR g.start_date >= CURRENT_DATE - (make_interval(days => $1))
    """
    return await fetch_all(query, within_date)
...
async def add_actual_progress(goal_id, amount, ref_date):
    ...
    """
    return await execute_query(query, goal_id, amount, ref_date)
