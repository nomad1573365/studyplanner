...
async def goal_name_autocomplete(
    ...
    tasks = sv.get_today_progresses(user_id, date.today())
    choices = []
    for task in tasks:
        goal_name = task[0]
...
@tree.command(name="reset_weight",description="重み付けをすべて1にリセットします")
async def command(interaction: discord.Interaction):
    await interaction.response.defer()
    user = rp.UserRepository()
    user_id = user.ensure_user(interaction.user.id)
    today = date.today()
    rp.delete_user_weight(user_id)
...
async def command(interaction: discord.Interaction):
    ...
    user = rp.UserRepository()
    user_id = user.ensure_user(interaction.user.id)
    if rp.delete_last_record(user_id):
        await interaction.followup.send(f":white_check_mark:正常に削除されました。")
    else:
        await interaction.followup.send(f":x:削除に失敗しました。やり直してください。")
