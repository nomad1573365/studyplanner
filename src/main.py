...
async def command(interaction: discord.Interaction, goal_name: str, start_point: int, end_point: int, start_date: str, end_date: str):
    ...
    if rp.add_goal(user_id, goal_name, start_point, end_point, start_date, end_date):
        await interaction.followup.send("ゴールが作成されました！")
    else:
        await interaction.followup.send("年月日を正しく入力し、やり直してください:sob:")
...
async def command(interaction: discord.Interaction, goal_name: str, amount: int):
    ...
    if add_plan:
        await interaction.followup.send(f"新規タスク、{ goal_name }({ amount })が今日の予定に追加されました！")
    else:
        await interaction.followup.send(f"予定を追加できませんでした。ゴールに{ goal_name }が存在するか確認した上、もう一度お試しください:sob:")
...
async def command(interaction: discord.Interaction, goal_name: str, amount: int):
    ...
    if sv.record_progress_by_name(user_id, goal_name, amount):
        await interaction.followup.send(f"{ goal_name }({ amount })が記録されました！")
    else:
        await interaction.followup.send(f"タスク{ goal_name }を記録できませんでした。予定に{ goal_name }が存在するか確認した上、もう一度お試しください:sob:")
...
async def command(interaction: discord.Interaction):
    ...
    await interaction.followup.send(formatter.show_daily_summary(task_list))
