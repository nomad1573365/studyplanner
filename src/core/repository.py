...
class UserRepository:
    async def get_by_id(self, user_id: int):
        ...
        _, username, _ = row
        return md.User(user_id, username)
...
async def record_actual_progress(goal_id, actual_amount, ref_date):
    ...
    status = await db.add_actual_progress(goal_id, actual_amount, ref_date)
    if status.endswith("0"):
        seed_plan = max(1, int(actual_amount))
        await db.add_plan(goal_id, seed_plan, ref_date)
        logger.info(
            f"Auto-created daily plan before recording progress: goal_id={goal_id}, "
            f"ref_date={ref_date}, seed_plan={seed_plan}"
        )
        status = await db.add_actual_progress(goal_id, actual_amount, ref_date)
        if status.endswith("0"):
            raise DomainError("進捗の反映に失敗しました。時間をおいて再度お試しください。")

    return status

async def undo_actual_progress(goal_id, amount, ref_date):
    if not goal_id:
        raise NotFoundError("指定のゴールが見つかりませんでした。")
    if amount is None or amount <= 0:
        raise DomainError("値が不正です。（amountは1以上にしてください）")

    status = await db.subtract_actual_progress(goal_id, amount, ref_date)
    if status.endswith("0"):
        raise DomainError("これ以上減らせません。")
    return status
...
async def add_daily_plan(user_id, goal_name, plan_amount):
    ...
    if plan_amount is None:
        raise DomainError("計画量が不正です。")
    try:
        normalized_amount = int(plan_amount)
    except (TypeError, ValueError):
        raise DomainError("計画量が不正です。")
    if normalized_amount <= 0:
        raise DomainError("計画量は1以上にしてください。")
    ...
    normalized_amount = min(normalized_amount, remaining)
    await db.add_plan(goal_id, normalized_amount, date.today())
    return True
...
async def delete_user(user_id):
    user = await PgUserRepository().find_by_id(user_id)
    if not user:
        raise NotFoundError("指定のユーザーが見つかりませんでした。")
...
class PgProgressRepository:
    ...
    async def fetch_progress(self, goal_id): # goal_id -> progress
        query = """
    ...
    async def get_dailytasks(self, user_id, ref_date) -> list[DailyTask]:
        query = """
            SELECT 
                ...
                p.plan_amount, 
                p.actual_amount,
                COALESCE(cumulative.current_point, 0) as current_point
            FROM progress_log p
            JOIN goals g ON p.goal_id = g.id
            LEFT JOIN (
                SELECT 
