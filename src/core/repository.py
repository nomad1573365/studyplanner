...
class PgObjectiveRepository:
    def _to_objective(self, user_id: int, objective: str):
        return Objective(user_id, objective)
    
    async def fetch_objective_by_userid(self, user_id: int) -> Objective:
        query = """
            SELECT objective
            FROM objective
            WHERE user_id = $1
        """
        objective = await fetch_val(query, user_id)
        return self._to_objective(user_id, objective) 
        
    async def save(self, objective: Objective) -> None:
        user_id = objective.user_id
        objective_str = objective.objective
        query = """
            INSERT INTO objective (user_id, objective)
                VALUES ($1, $2)
                ON CONFLICT (user_id)
                DO UPDATE SET 
                    objective = EXCLUDED.objective
        """
        return await execute_query(query, user_id, objective_str)