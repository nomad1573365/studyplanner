...
async def fetch_active_userlist(within_date, ref_date=None):
    if ref_date is None:
        ref_date = date.today()
    query = """
        ...
        LEFT JOIN progress_log p ON g.id = p.goal_id
        WHERE p.date >= $2 - (make_interval(days => $1))
           OR g.start_date >= $2 - (make_interval(days => $1))
    """
    return await fetch_all(query, within_date, ref_date)
...
async def subtract_actual_progress(goal_id, amount, ref_date):
    query = """
        UPDATE progress_log
        SET actual_amount = COALESCE(actual_amount, 0) - $1
        WHERE goal_id = $2
          AND date = $3
          AND COALESCE(actual_amount, 0) >= $1
    """
    return await execute_query(query, amount, goal_id, ref_date)
...
async def delete_last_record(user_id):
    ...
    try:
        ...
        logger.error(
            f"delete_last_record failed: user_id={user_id}, error={e}",
            exc_info=(type(e), e, e.__traceback__),
        )
        raise
