...
def record_actual_progress(goal_id, actual_amount):
    try:
        return db.add_actual_progress(goal_id, actual_amount)
...
def get_user_goal_list(user_id, ref_date=date.today()): # [["id", "ゴール名", "開始地点", "終了地点", "開始日", "終了日", "現在地点"], ...]
    goal_list = []
...
def delete_user_weight(user_id):
    goal_list = db.fetch_goal_with_progress(user_id)
    print(goal_list)
    for goal in goal_list:
        goal_id = goal[0]
        db.delete_weights(goal_id)

def delete_last_record(user_id):
    return db.delete_last_record(user_id)
