...
class DashboardCog(commands.Cog):
    ...
    async def _generate_dashboard_unlocked(self, user_id, msgid_snapshot=None):
        ...
        for task in task_list:
            ...
            status_msg = await user.send(discord_ui.task_status(task), view=view)
            await dmr.update(status_msg.id, user_id, "TASK", task.goal_id)
    ...
    async def on_update_dashboard(self, goal_id: int):
        try:
            user = await rp.PgUserRepository().find_by_goal(goal_id)
            ...
            if lock.locked():
                ...
                return
            async with lock:
                task_msg_id = await dmr.fetch_msgid_by_goal_id(goal_id, "TASK")
                if not task_msg_id:
                    logger.warning(
                        f"DASHBOARD UPDATE fallback generate: TASK msg missing user_id={user_id}, goal_id={goal_id}"
                    )
                    self.bot.dispatch("generate_dashboard", user_id)
                    return

                try:
                    task_msg = await channel.fetch_message(task_msg_id)
                except discord.NotFound:
                    logger.warning(
                        f"DASHBOARD UPDATE fallback generate: TASK msg not found user_id={user_id}, goal_id={goal_id}"
                    )
                    self.bot.dispatch("generate_dashboard", user_id)
                    return
                ...
                for task in task_list:
                    if task.goal_id == goal_id:
                        await task_msg.edit(content=discord_ui.task_status(task))
