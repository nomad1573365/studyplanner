...
class DashboardCog(commands.Cog):
    ...
    async def _delete_dashboard_messages(self, channel, dmr, user_id, msgid_list=None):
    ...
    async def _generate_dashboard_unlocked(self, user_id, msgid_snapshot=None):
        ...
        await self._delete_dashboard_messages(channel, dmr, user_id, msgid_snapshot)
    ...
    async def on_delete_dashboard(self, user_id):
        try:
            ...
            async with lock:
                await self._delete_dashboard_messages(channel, dmr, user_id)
        except Exception as e:
            logger.error(
...
class TaskButton(discord.ui.View):
    ...
    async def minus_1(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message("-# 処理中",ephemeral=True,delete_after=0.1)
        try:
            await sv.undo_progress_by_id(self.goal_id, 1)
            self.cog.bot.dispatch("update_dashboard", self.goal_id)
        except DomainError as e:
            await interaction.followup.send(str(e), ephemeral=True, delete_after=3)
        except Exception as e:
