...
def show_goals_summary(goal_list: list[Goal]):
    if not goal_list:
        return "まだゴールはないようです！"
    title = ""
    content_unfinished = ""
    ...
    for goal in goal_list:        
        total_amount = goal.end_point - goal.start_point + 1
        ratio = goal.current_point / total_amount
        book_icon = progress_to_bookicon(ratio)
        remaining_days = goal.end_date - date.today()
        if ratio < 1:
            rows = [f"{book_icon} **{goal.goal_name}**",
                    f"    **進捗** `{make_progressbar(ratio, 16)}`",
                    f"    **現在 {goal.current_point}/{total_amount} pts**",
                    f"    **完了予定 {goal.end_date.strftime('%m/%d')}（{format_timedelta(remaining_days)}）**"]
            content_unfinished += "\n".join(rows) + "\n\n"
        else:
            rows = [f"{book_icon} **{goal.goal_name} --- 完了！！**\n"]
            content_finished += "\n".join(rows)
    content = content_unfinished + content_finished
    footer = ""
    return message_configuration(title, content, footer)
    
def show_daily_summary(task_list: list[DailyTask], ref_date: date):
    if not task_list:
    ...
    for task in task_list:
        ratio = task.actual_amount / task.plan_amount if task.plan_amount > 0 else 0
        
        row1 = f"{progress_to_bookicon(ratio)} **{task.goal_name} ------ {task.actual_amount}/{task.plan_amount}**\n"
        row2 = f"          **今日やること:  {task.today_start_point}〜{task.today_start_point+task.plan_amount-1}**\n"
...
def task_status(task: DailyTask) -> str:
    progress_ratio = task.actual_amount / task.plan_amount if task.plan_amount > 0 else 0
    status = ":white_check_mark: **完了！**" if task.is_completed else ":fire: **取組中**"
    current_point = f"({str(task.today_start_point + task.actual_amount - 1)}まで記録済み)" if task.actual_amount != task.plan_amount else ""
    row1 = f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    row2 = f"{progress_to_bookicon(progress_ratio)} **{task.goal_name} ------ {task.actual_amount}/{task.plan_amount}**    {status}\n"
    row3 = f"**今日やること:  {task.today_start_point}〜{task.today_start_point+task.plan_amount-1} {current_point}**\n"
    row4 = f"**進捗:**  `{make_progressbar(progress_ratio, 16)}`\n"
...
def top_message(tasks: list[DailyTask]):
    ...
    if not tasks:
        ...
        total_plan = sum(max(t.plan_amount, t.actual_amount) for t in tasks)
        done_count = sum(1 for t in tasks if t.is_completed)
        total_count = len(tasks)
        
        ratio = total_actual / total_plan if total_plan > 0 else 0
        bar = make_progressbar(ratio, 18)
        
        icon = ":tada:" if total_actual >= total_plan else ":books:"
        ...
        content = (
            ...
            f"**完了済み:  {done_count} / {total_count}**  {icon}\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"-# LAST UPDATE: <t:{now_unixtime}:R> "
        )
    return content
